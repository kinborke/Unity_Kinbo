...
public class PlayerController : MonoBehaviour
{
    ...
    private void swipe()
    {
        if (Input.GetMouseButton(0))
        {
            ...
            float z;
            z = Mathf.Clamp(deltaX, -45, 45);
            transform.GetChild(0).rotation = Quaternion.Slerp(transform.GetChild(0).rotation, Quaternion.Euler(transform.GetChild(0).eulerAngles.x,z*10, transform.GetChild(0).eulerAngles.z), Time.deltaTime * 11);
